    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.targets.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('num_of_orders') ? 'is-invalid' : '' }}" type="number" name="num_of_orders" id="num_of_orders" value="{{ old('num_of_orders', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('delegates') ? 'is-invalid' : '' }}" name="delegates[]" id="delegates" multiple required>
                    @foreach($delegates as $id => $delegate)
                        <option value="{{ $id }}" {{ in_array($id, old('delegates', [])) ? 'selected' : '' }}>{{ $delegate }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Target::PROFIT_TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('profit_type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="profit_type_{{ $key }}" name="profit_type" value="{{ $key }}" {{ old('profit_type', '') === (string) $key ? 'checked' : '' }} required>
