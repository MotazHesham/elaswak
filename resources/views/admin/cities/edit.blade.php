        ...
        <form method="POST" action="{{ route("admin.cities.update", [$city->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('district') ? 'is-invalid' : '' }}" name="district_id" id="district_id" required>
                    @foreach($districts as $id => $entry)
                        <option value="{{ $id }}" {{ (old('district_id') ? old('district_id') : $city->district->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('delivery') ? 'is-invalid' : '' }}">
                    <input class="form-check-input" type="checkbox" name="delivery" id="delivery" value="1" {{ $city->delivery || old('delivery', 0) === 1 ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('delivery_cost') ? 'is-invalid' : '' }}" type="number" name="delivery_cost" id="delivery_cost" value="{{ old('delivery_cost', $city->delivery_cost) }}" step="0.01" required>
